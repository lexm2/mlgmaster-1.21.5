...
public class WaterMLGHandler {
    ...
    public static void onHighFrequencyTick() {
        ...
        if (prediction.isUrgentPlacement()) {
            MLGMaster.LOGGER.warn("🚨 URGENT MLG SITUATION: {} blocks to target!", prediction.getDistanceToTarget());
        }
        ...
        if (prediction.shouldPlace()) {
            ...
            MLGMaster.LOGGER.info("  Placement threshold: {:.3f} blocks", prediction.getPlacementDistance());
            MLGMaster.LOGGER.info("  Distance to target: {:.3f} blocks", prediction.getDistanceToTarget());
